function deleteAndDisapper(e) {
    $.ajax({
        ...
        'url': "http://localhost:3000/order/deleteOrder/" + e,
    });
}
...
function addOrderCard(obj) {
    ...
    let templateString3 = '</tbody></table><div class="d-flex justify-content-center"><button class="btn btn-outline-dark mr-4" type="button" id="accept"'+obj.orderId+' >Accept</button><a  onclick="deleteAndDisapper('+obj.orderId+')" href="#' + obj.orderId + '" class="btn btn-outline-dark"style="marigin-left:50px">Done</a></div></div></div></div>';
...
function apiCall(currentOrders)
{
    $.get('http://localhost:3000/order/fetchOrders', (obj, status) => {
...
$(document).ready(() => {
    ...
    var currentOrders = [];
    setInterval(()=>apiCall(currentOrders), 1000);
