...
function addOrderToRestaurant(newOrders)
{
    for(obj of newOrders)
    {
        ...
        let templateString3 = '</tbody></table><div class="d-flex justify-content-center"><a class="btn btn-outline-success bgl-success btn-block" onclick="deleteAndDisapper('+obj.orderId+')" href="#' + obj.orderId + '" >Done</a></div></div></div></div>';
...
async function getAcceptedapiCall(currentOrders)
{
    ...
    await $.ajax({
        method:'POST',
        url:"http://localhost:3000/order/fetchRestaurantOrders/"+restaurant_id+"/"+status,
